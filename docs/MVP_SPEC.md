...
top. Actions: list (defaults + own), create own, delete own (defaults can't be
deleted).
...
Category      — id, user_id? (null = shared default), name, created_at
Task          — id, user_id, project_id?, category_id?, title, description,
                status, priority, estimated_duration, deadline,
                created_at, completed_at
Project       — id, user_id, name, description, status, deadline
